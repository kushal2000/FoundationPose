...
def read_rgbd_frame(zed, runtime_parameters, image_mat, depth_mat, width, height, camera_upsidedown=False):
    ...
    # imageio.imwrite(f'rgb_0.png', img)
...
def main():
    ...
    parser.add_argument('--track_refine_iter', type=int, default=5, help='Refinement iterations per tracking step')
    parser.add_argument('--debug', type=int, default=0, help='Debug level for FoundationPose')
    parser.add_argument('--save_dir', type=str, default='live_tracking_results', help='Directory to save tracking results')
    ...
    # Initialize ROS node
    rospy.init_node('live_tracking_ros', anonymous=True)

    # Initialize publisher
    pose_pub = rospy.Publisher('/object_pose', PoseStamped, queue_size=10)
    robot_frame_pub = rospy.Publisher('/robot_frame', PoseStamped, queue_size=10)
    ...
    T_RC = np.array([
        ...
        [-0.04369193087682233, -0.90494235937159795, -0.42328517738189414, 1.02518932829980036],
        [0.00000000000000000, 0.00000000000000000, 0.00000000000000000, 1.00000000000000000]
    ])
    ...
    try:
        ...
        # Load mesh (assumed meters; allow optional scaling)
        ...
        while True:
            ...
            # imageio.imwrite(f'rgb_OURS.png', rgb)
            # breakpoint()
            if args.activate_2d_tracker:
            ...
            pose_msg = pose_matrix_to_posestamped(pose, 'object_frame')
            pose_pub.publish(pose_msg)
            robot_frame_msg = pose_matrix_to_posestamped(T_RC, 'robot_frame')
            robot_frame_pub.publish(robot_frame_msg)
            # Visualization
            if pose is not None and args.debug>=1:
                ...
                if key in [ord('q'), 27]:  # q or ESC
                    break
                # if key == ord('r'):
                #     # Re-register with a new SAM-generated mask on the current frame
                #     mask = select_mask_with_sam(rgb)
                #     if mask.sum() > 0:
                #         print('Re-registering...')
                #         pose = est.register(K=K, rgb=rgb, depth=depth, ob_mask=mask.astype(bool), iteration=args.est_refine_iter)
                #     else:
                #         print('Empty ROI; ignoring re-register.')
