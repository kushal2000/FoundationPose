...
def main():
    ...
    try:
        ...
        while True:
            if is_first_frame:
                import imageio
                if len(rgb_array) > 0 and len(depth_array) > 0 and len(subgoals_array) > 0:
                    # Rgb video
                    # trajectory_name = input('Enter trajectory name: ')
                    current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                    # traj_save_folder = os.path.join(save_folder, trajectory_name)
                    traj_save_folder = f"{save_folder}/{current_time}"
                    os.makedirs(traj_save_folder, exist_ok=True)
                    imageio.mimsave(f'{traj_save_folder}/rgb_video.mp4', rgb_array, fps=30)

                    # Rgb and depth files
                    rgb_folder = os.path.join(traj_save_folder, 'rgb')
                    depth_folder = os.path.join(traj_save_folder, 'depth')
                    os.makedirs(rgb_folder, exist_ok=True)
                    os.makedirs(depth_folder, exist_ok=True)
                    for idx, (rgb, depth) in enumerate(zip(rgb_array, depth_array)):
                        rgb_save_path = os.path.join(rgb_folder, f'frame_{idx:04d}.png')
                        depth_save_path = os.path.join(depth_folder, f'frame_{idx:04d}.png')
                        cv2.imwrite(rgb_save_path, cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR))
                        # (depth_image*1000).astype(np.uint16)
                        cv2.imwrite(depth_save_path, (depth*1000).astype(np.uint16))

                    # Subgoals file
                    with open(os.path.join(traj_save_folder, 'subgoals.json'), 'w') as f:
                        json.dump(subgoals_array, f)

                    with open(os.path.join(traj_save_folder, 'cam_K.txt'), 'w') as f:
                        np.savetxt(f, K)
                ...
                subgoals_array.append([
                    robot_frame_pose.pose.position.x, 
                    robot_frame_pose.pose.position.y, 
                    robot_frame_pose.pose.position.z, 
                    robot_frame_pose.pose.orientation.x, 
                    robot_frame_pose.pose.orientation.y,
                    robot_frame_pose.pose.orientation.z,
                    robot_frame_pose.pose.orientation.w
                ])
