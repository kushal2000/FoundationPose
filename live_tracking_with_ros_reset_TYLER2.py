...
# Must be set before importing cv2 so Qt uses a headless backend
# (avoids "could not connect to display" crash on headless machines).
...
from generate_mask import BoundingBox, segment_with_box
import sys
...
# ---------------------------------------------------------------------------
# Signal-based headless control (replaces cv2 keyboard interaction)
#   pkill -f -SIGUSR1 'live_tracking_with_ros_reset_TYLER'  → reset   (was 'r')
#   pkill -f -SIGUSR2 'live_tracking_with_ros_reset_TYLER'  → delete  (was 'd')
#   pkill -f -SIGINT  'live_tracking_with_ros_reset_TYLER'  → quit    (was 'q'/ESC)
# ---------------------------------------------------------------------------
_flag_reset = False
_flag_delete_buffer = False
_flag_quit = False


def _handle_reset(signum, frame):
    global _flag_reset
    _flag_reset = True
    print("\n[Signal] Received SIGUSR1 – will reset pose tracking.")


def _handle_delete_buffer(signum, frame):
    global _flag_delete_buffer
    _flag_delete_buffer = True
    print("\n[Signal] Received SIGUSR2 – will delete buffer.")


def _handle_quit(signum, frame):
    global _flag_quit
    _flag_quit = True
    print(f"\n[Signal] Received {signal.Signals(signum).name} – will quit.")
...
def select_mask_with_sam(rgb):
    """
    Generate a SAM mask using a hardcoded bounding box (no GUI needed).
    Box: x=555, y=278, width=257, height=128.
    """
    ...
    box = BoundingBox(
        xmin=HARDCODED_X,
        ymin=HARDCODED_Y,
        xmax=HARDCODED_X + HARDCODED_W,
        ymax=HARDCODED_Y + HARDCODED_H,
    )
    pil_image = PILImage.fromarray(rgb)  # rgb is already RGB
    mask = segment_with_box(pil_image, box, polygon_refinement=True)
    if mask is None:
        return None
...
def main():
    ...
    # Register signal handlers for headless control
    signal.signal(signal.SIGUSR1, _handle_reset)
    signal.signal(signal.SIGUSR2, _handle_delete_buffer)
    signal.signal(signal.SIGINT, _handle_quit)
    signal.signal(signal.SIGTERM, _handle_quit)

    _SCRIPT_NAME = "live_tracking_with_ros_reset_TYLER"
    print(f"PID {os.getpid()} – headless signal controls:")
    print(f"  pkill -f -SIGUSR1 '{_SCRIPT_NAME}'  → reset pose tracking")
    print(f"  pkill -f -SIGUSR2 '{_SCRIPT_NAME}'  → delete replay buffer")
    print(f"  pkill -f -SIGINT  '{_SCRIPT_NAME}'  → quit")
    ...
    try:
        ...
        while True:
            if is_first_frame:
                ...
                # --- Check signal flags (headless control) ---
                global _flag_reset, _flag_quit, _flag_delete_buffer
                if _flag_reset:
                    _flag_reset = False
                    is_first_frame = True
