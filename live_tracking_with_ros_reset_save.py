        ...
        while True:
            if is_first_frame:
                ...
                if len(rgb_array) > 0 and len(depth_array) > 0 and len(subgoals_array) > 0:
                    # Rgb video
                    current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                    imageio.mimsave(f'{save_folder}/{current_time}.mp4', rgb_array, fps=30)
                    ...
                    for rgb, depth in zip(rgb_array, depth_array):
                        rgb_save_path = os.path.join(rgb_folder, f'frame_{current_video_idx:04d}.png')
                        depth_save_path = os.path.join(depth_folder, f'frame_{current_video_idx:04d}.png')
                        cv2.imwrite(rgb_save_path, cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR))
                        cv2.imwrite(depth_save_path, depth)

                    # Subgoals file
                    with open(os.path.join(save_folder, 'subgoals.json'), 'w') as f:
